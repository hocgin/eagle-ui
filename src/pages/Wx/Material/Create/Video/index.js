...
@connect(({ global, wxMpMaterial: { paging }, loading, ...rest }) => {
  return {
    confirmLoading: loading.effects['wxMpMaterial/uploadVideo'],
  };
}, dispatch => ({
  $uploadVideo: (args = {}) => dispatch({ type: 'wxMpMaterial/uploadVideo', ...args }),
}))
class index extends React.Component {
      ...
      <Card>
        <Form ref={this.createForm}>
          ...
          <Form.Item wrapperCol={{ span: 13, offset: 7 }}>
            <video width={400} height={300} style={{ marginTop: 10 }} src={url} controls={true}/>
          </Form.Item>
  ...
  onDone = () => {
    let { $uploadVideo, location: { query: { appid } } } = this.props;
    let { url } = this.state;
