...
import WxMpQrcodeApi from '@/services/WxMpQrcode';

export default {
  namespace: 'wxMpQrcode',
  state: {
    paging: null,
    detail: null,
    all: [],
  ...
  effects: {
    ...
    * paging({ payload = {}, callback }, { call, put }) {
      let result = yield WxMpQrcodeApi.paging(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
    ...
    * getOne({ payload = {}, callback }, { call, put }) {
      let result = yield WxMpQrcodeApi.getOne(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
    ...
    * insert({ payload = {}, callback }, { call, put }) {
      let result = yield WxMpQrcodeApi.insert(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
        if (callback) callback(result);
      }
    },
