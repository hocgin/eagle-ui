...
import WxMpShortUrlApi from '@/services/WxMpShortUrl';

export default {
  namespace: 'wxMpShortUrl',
  state: {
    paging: null,
    detail: null,
    all: [],
  ...
  effects: {
    ...
    * paging({ payload = {}, callback }, { call, put }) {
      let result = yield WxMpShortUrlApi.paging(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
    ...
    * getOne({ payload = {}, callback }, { call, put }) {
      let result = yield WxMpShortUrlApi.getOne(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
    ...
    * insert({ payload = {}, callback }, { call, put }) {
      let result = yield WxMpShortUrlApi.insert(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
        if (callback) callback(result);
      }
    },
