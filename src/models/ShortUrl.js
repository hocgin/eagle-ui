...
import ShortUrlApi from '@/services/ShortUrl';
...
export default {
  ...
  effects: {
    ...
    * paging({ payload = {}, callback }, { call, put }) {
      let result = yield ShortUrlApi.paging(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
    ...
    * insert({ payload = {}, callback }, { call, put }) {
      let result = yield ShortUrlApi.insert(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
        if (callback) callback(result);
      }
    },
    ...
    * update({ payload = {}, callback }, { call, put }) {
      let result = yield ShortUrlApi.update(payload); // API
      if (UiUtils.showErrorMessageIfExits(result)) {
        if (callback) callback(result);
      }
    },
